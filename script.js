alert("ПРИВЕТСТВУЕМ ВАС в нашей Уютной ферме!");
...
jQuery('document').ready(function(){
	...
	var y;
	y=prompt('Напишите ваше имя?','Олег');
	$('#e4').html('Вам обязательно понравится у нас, '+y);
	confirm('Готовы получить море впечатлений?');
	var f6;
		f6= 170;
		if (f6>150||f6==150){
		alert('Наш запас зерна кукурузы большой');
		}
		else if(f6==149){
		alert('У нас есть определенный запас зерна кукурузы');
		}
		else{
		alert('Торопитесь, наш запас зерна кукурузы кончается');
		}
